...
NAN_METHOD(divbwt_) {
  ...
  size_t tLen = Buffer::Length(tObj);
  size_t uLen = Buffer::Length(uObj);
  size_t aLen = Buffer::Length(aObj);
  ...
  sauchar_t *T = reinterpret_cast<sauchar_t *>(Buffer::Data(tObj));
  sauchar_t *U = reinterpret_cast<sauchar_t *>(Buffer::Data(uObj));
  saidx_t *AUX = reinterpret_cast<saidx_t *>(Buffer::Data(aObj));
